...
const isValidTheme = (v) => typeof v === "string" && themes.includes(v);
...
// Expose a small API for dev tooling / debugging.
window.__lunaTheme = {
  themes: [...themes],
  get: () => getCurrentTheme(),
...
// Toggle only between light and the default dark.
const toggleButton = document.getElementById("theme-toggle");
if (toggleButton) {
...
// Theme dropdown (Shift + T)
// Hidden by default; toggled via shortcut.
{
  const labelFor = (t) => {
  ...
  window.addEventListener("keydown", (e) => {
    ...
    const wantsToggle = !isEditable && key === "t" && e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey;
